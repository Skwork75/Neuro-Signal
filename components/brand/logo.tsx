...
export function BrandLogo({ className, markClassName, showName = true, nameClassName }: BrandLogoProps) {
  return (
    <span className={cn("inline-flex items-center gap-2.5", className)}>
      <span className={cn("flex size-9 items-center justify-center rounded-xl bg-emerald-700 text-emerald-50 shadow-sm shadow-emerald-900/20", markClassName)}>
      ...
      {showName ? <span className={cn("font-semibold tracking-tight text-slate-950 dark:text-emerald-50", nameClassName)}>Neuro<span className="text-emerald-700 dark:text-emerald-300">Signal</span></span> : null}
    </span>
  );
}
