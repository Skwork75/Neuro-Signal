...
export default function CounselorWorkspace({ entries }: { entries: Entry[] }) {
  ...
  if (result) {
    return <div className="mx-auto max-w-4xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <Button type="button" variant="outline" onClick={() => setResult(null)}>Start over</Button>
      </div>
  ...
  return <div className="mx-auto max-w-4xl space-y-6"><div><p className="flex items-center gap-2 text-sm font-semibold uppercase tracking-[0.14em] text-emerald-600 dark:text-emerald-400"><Brain className="size-4" />Guided support</p><h1 className="mt-2 text-3xl font-semibold text-slate-900 dark:text-emerald-50">Talk it through</h1><p className="mt-2 max-w-2xl text-sm leading-6 text-slate-600 dark:text-emerald-100/70">Answer a few gentle questions. You can bring in moments from your journal or add context that has not been written down yet.</p></div>
